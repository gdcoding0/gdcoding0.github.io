...
function loadSequence() {
  ...
  const findPage = find((pages) => pages.title === "landing");
  if (findPage) {
    // Find index of 'landing' page
    const findIndex = findIndex((pages) => pages === findPage);
...
function appNav(current, next) {
  ...
  if (getCurrent === current) {
    ...
    const findPage = find((pages) => pages.title === next);
    if (findPage) {
      // Find index for corresponding page in 'pages' array
      const findIndex = _findIndex((pages) => pages === findPage);
